...
def herfindahl_hirschman_index(w: pd.Series) -> float: 
    return float((w.fillna(0.0)**2).sum())
...
def variance_contributions(returns: pd.DataFrame, w: pd.Series) -> pd.Series:
    """Risk (variance) contribution per position."""
    ...
    mrc = np.dot(cov, wv)  # marginal risk contributions
    rc = (wv * mrc) / port_var
